...
if (window.supabase) {
    ...
    const updateProfileUI = (user) => {
        ...
        // Update all avatars
        document.querySelectorAll('.bg-center.bg-no-repeat.aspect-square.bg-cover.rounded-full, #profile-avatar-large').forEach(img => {
            if (metadata.avatar_url) img.style.backgroundImage = `url('${metadata.avatar_url}')`;
        });

        // Update all name headers/labels
        document.querySelectorAll('.text-2xl.font-bold, .text-deep-charcoal.text-xl.font-bold, #profile-name').forEach(el => {
            el.innerText = metadata.full_name || user.email.split('@')[0];
        });
    ...
    window.supabaseClient.auth.onAuthStateChange((event, session) => {
        if (session && session.user) {
            updateProfileUI(session.user);
            localStorage.removeItem('isGuest'); // Login clears guest mode
        }
    });
}
...
async function uploadMedia(file) {
    ...
    try {
        ...
        if (!response.ok) {
            alert("Make sure your local server is running! (node server.js)");
